...
test.describe("Task Management", () => {
  ...
  test("should reorder tasks within column and between columns", async ({ page }) => {
    ...
    const doingTask2CardFinal = doingColumn.getByRole("button", {
      name: /Open task Doing Task 2/i,
    });
    ...
    await expect(
      toDoColumnReloaded.getByRole("button", { name: /Open task Task 2/i }),
    ).toBeVisible();
    await expect(
      toDoColumnReloaded.getByRole("button", { name: /Open task Task 1/i }),
    ).not.toBeVisible();
    await expect(
      toDoColumnReloaded.getByRole("button", { name: /Open task Task 3/i }),
    ).not.toBeVisible();
